...
const ForeignExchange: React.FC<State> = (props: State) => {
    ...
    return pockets
      ...
      .map(pocket => (
        <option key={pocket.currency.id} value={pocket.currency.id}>
          {singleInput.toString()} = {singleInput.toExchange(state.rates[pocket.currency.id], pocket.currency.id).toString()}
        </option>
      ))
  }
  ...
  const exchange = () => {
    dispatch(actionCreators.exchange({
      input: state.input,
      output: state.output,
    }));
  }
    ...
    <React.Fragment>
      <Card>
        ...
        <div className="space-between">
          ...
          <InputAmount setValue={(value: number) => dispatch(actionCreators.setInputAmount(value))} amount={state.input} />
        </div>
      ...
      <Card color={CardColor.Dark}>
        ...
        <div className="space-between">
          ...
          <Text>
            {singleOutput.toString()} = {singleOutput.toExchange(1/state.rates[state.output.currency.id], state.input.currency.id).toString()}
          </Text>
        </div>
        <button
          type="button"
